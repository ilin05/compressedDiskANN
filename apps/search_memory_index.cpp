                        ...
                        const bool use_pq_dist, const uint32_t pq_bytes, const std::string &pq_prefix, const float pq_exact_rerank_ratio)
{
    ...
    auto config = diskann::IndexConfigBuilder()
                      ...
                      .is_pq_dist_build(use_pq_dist)
                      .is_use_opq(false)
                      .with_num_pq_chunks(pq_bytes)
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        csv_rows.push_back(
            SearchCsvRow{L, displayed_qps, avg_cmps, static_cast<float>(mean_latency),
                         static_cast<float>(latency_stats[(uint64_t)(0.999 * query_num)]), recall_for_csv,
                         calc_recall_flag});
    }
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        optional_configs.add_options()("pq_prefix",
                                       po::value<std::string>(&pq_prefix)->default_value(""),
                                       "Prefix of the PQ files, usually original input data file path.");
